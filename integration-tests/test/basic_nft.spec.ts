...
describe('NFT Test\n', async () => {
  ...
  it('{tag:nft} should have a name', async () => {
    const tokenName = await ERC721.name()
    expect(tokenName).to.equal('TestNFT')
  })

  it('{tag:nft} should generate a new ERC721 and transfer it from Bob (a1a) to Alice (a2a)', async () => {
  ...
  it('{tag:nft} should derive an NFT Factory from a genesis NFT', async () => {
  ...
  it('{tag:nft} should register the NFTs address in users wallet', async () => {
